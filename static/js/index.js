...
var searchFunc = function() {
    ...
    if( !$("#files-info").val() || $("#files-info").val().length < 0 ) {
        $("#files-info").focus().val("").attr("placeholder","搜索内容不能为空或长度小于0个字符");
...
$('.search-text-icon').click(searchFunc)
$('#files-info').keydown(function(event){
    if(event.which == 13) {
        searchFunc()
    }
})
